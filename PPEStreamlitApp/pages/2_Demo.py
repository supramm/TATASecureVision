...
<div class="ppe-header">
    ...
    <p>// OFFLINE VIDEO ANALYSIS · YOLOV8 INFERENCE ENGINE · TATAVISION SECURE</p>
</div>
""", unsafe_allow_html=True)
...
# ── Demo video registry ────────────────────────────────────────────────────────
DEMO_VIDEOS = [
    {"id": "demo1", "icon": "🏗️", "title": "Construction Site", "desc": "Outdoor scaffold zone",   "file": "construction_site.mp4"},
    {"id": "demo2", "icon": "🏭", "title": "Factory Floor",     "desc": "Heavy machinery area",    "file": "factory_floor.mp4"},
    {"id": "demo3", "icon": "🧪", "title": "Lab Environment",   "desc": "Chemical handling zone",  "file": "lab_env.mp4"},
    {"id": "demo4", "icon": "🚧", "title": "Road Works",        "desc": "Traffic-adjacent site",   "file": "road_works.mp4"},
]
...
with left:
    ...
    source_mode = st.radio("Source", ["📁 Upload a video", "🎬 Use a demo video"],
    ...
    if source_mode == "📁 Upload a video":
        ...
        uploaded_file = st.file_uploader("Drop your video here",
        ...
        for dv in DEMO_VIDEOS:
            ...
            tag       = "✔ " if is_sel else ""
            note      = "" if available else " *(file missing)*"
            if st.button(f"{dv['icon']} {tag}{dv['title']}{note} — {dv['desc']}",
                         key=f"btn_{dv['id']}",
...
with right:
    ...
    if st.session_state.running:
        ...
        if not cap.isOpened():
            st.error("❌ Could not open video. Check file path or re-upload.")
            st.session_state.running = False
        ...
        while cap.isOpened() and st.session_state.running:
            ...
            annotated = results.plot(labels=show_labels) if show_boxes else frame.copy()
            ...
            if "Person" in detected:
                ...
                if frame_violations:
                    total_violations += 1
                    for i, v in enumerate(frame_violations):
                        cv2.putText(annotated, f"! {v}", (10, 30 + i * 30),
                    ...
                    total_compliant += 1
                    if len(compliance_log) < 50:
                        compliance_log.append((ts, "All PPE compliant"))

            h, w = annotated.shape[:2]
            if w > 760:
                annotated = cv2.resize(annotated, (760, int(h * 760 / w)))
            ...
            <div class="metric-row">
                <div class="metric-tile"><span class="val val-blue">{processed}</span><div class="lbl">Frames Analysed</div></div>
                <div class="metric-tile"><span class="val val-blue">{total_persons}</span><div class="lbl">Person Detections</div></div>
                <div class="metric-tile"><span class="val val-err">{total_violations}</span><div class="lbl">Violation Frames</div></div>
                <div class="metric-tile"><span class="val val-ok">{total_compliant}</span><div class="lbl">Compliant Frames</div></div>
                <div class="metric-tile"><span class="val {c_color}">{compliance_rate}%</span><div class="lbl">Compliance Rate</div></div>
            </div>
            """, unsafe_allow_html=True)

            vlog_html = "".join(
                f'<div class="vlog-entry"><span>{t}</span>{v}</div>'
                for t, v in violation_log[-8:][::-1]
            ) or '<div style="color:#334155;font-family:\'Share Tech Mono\',monospace;font-size:0.75rem;">No violations yet</div>'

            clog_html = "".join(
                f'<div class="clog-entry"><span>{t}</span>{v}</div>'
                for t, v in compliance_log[-8:][::-1]
        ...
        if processed > 0:
            ...
            st.markdown(f"""
            <div style="background:#111827;border:1px solid #1e2d45;border-radius:10px;padding:24px 28px;">
                ...
                <div style="font-family:'Share Tech Mono',monospace;font-size:0.8rem;
                            color:#64748b;line-height:1.9;margin-bottom:12px;">
                ...
                <div style="font-family:'Share Tech Mono',monospace;font-size:0.7rem;
                            color:#334155;border-top:1px solid #1e2d45;padding-top:10px;">
